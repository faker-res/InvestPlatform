...
cd ${APP_HOME}/${PROJECT_NAME}
#java -server -jar ${APP_HOME}/${PROJECT_NAME}/${FILE_NAME} --spring.profiles.active=${ENV} >/dev/null 2>&1 &
java -server -jar -Xms1024m -Xmx1024m ${APP_HOME}/${PROJECT_NAME}/${FILE_NAME} --spring.profiles.active=${ENV} >${APP_HOME}/${PROJECT_NAME}/logs/runtime.log 2>&1 &
