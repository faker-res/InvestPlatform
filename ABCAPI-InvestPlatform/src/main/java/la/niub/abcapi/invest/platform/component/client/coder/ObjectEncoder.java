package la.niub.abcapi.invest.platform.component.client.coder;
...
public class ObjectEncoder implements Encoder {
    ...
    public void encode(Object o, Type type, RequestTemplate requestTemplate) throws EncodeException {
        ...
        Boolean requestBody = params.containsKey("json");
        ...
        if (requestTemplate.method().equals("POST") && requestBody){
            String jsonStr = JSON.toJSONString(querysStr);
            requestTemplate.header("Content-Type", MediaType.APPLICATION_JSON_VALUE);
            requestTemplate.body(jsonStr);
        }else if (requestTemplate.method().equals("POST") && !requestBody){
        ...
        if (includeParent){
            ...
            while (!parent.equals(Object.class)){
                ...
                for (Field field : fields){
                    try {
                        field.setAccessible(true);
                        map.putIfAbsent(field.getName(),field.get(bean));
                    } catch (IllegalAccessException e) {
                        e.printStackTrace();
                    }
                }
