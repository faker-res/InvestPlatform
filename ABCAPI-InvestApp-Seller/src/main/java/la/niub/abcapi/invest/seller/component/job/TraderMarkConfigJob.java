...
import la.niub.abcapi.invest.seller.model.response.client.AnalystReportCountResponse;
 ...
 */
@Component
public class TraderMarkConfigJob {
    ...
    @Scheduled(cron = "0 0 0 2 1,4,7,10 ?")
    @Async
    public void produceTraderMarkJob() {
        ...
        try {
            ...
            for (int i = 0; i < parentIdList.size(); i++) {
                ...
                for (TraderMarkConfigObjectModel configObjectModel : configObjectModelList) {
                    ...
                    // 获取研报量接口
                    Integer reportCount = getReportCount(parentId, configObjectModel.getBroker(), configObjectModel.getAnalyst(), year, quarter);
                    // 获取阅读量接口
                    Integer readingCount = getReadingCount(parentId, configObjectModel.getAnalyst(), year, quarter);
                    // 获取平均完成天数接口
                    BigDecimal attainDays = getAttainDays(parentId, configObjectModel.getBroker(), configObjectModel.getAnalyst(), year, quarter);
                    // 获取达成率
                    BigDecimal attainProbability = getAttainProbability(parentId, configObjectModel.getBroker(), configObjectModel.getAnalyst(), year, quarter);
                    // 获取路演数量
    ...
    private Integer getReportCount(String parentId, String broker, String analyst, Integer year, Integer quarter) {
    ...
    private Integer getReadingCount(String parentId, String analyst, Integer year, Integer quarter) {
    ...
    private BigDecimal getAttainDays(String parentId, String broker, String analyst, Integer year, Integer quarter) {
        // analyst的名称需要去除末尾的数字，因为券商评分的同券商同行业不允许出现重名，如果重名在后面加数字
        try {
            QuarterDateEnum quarterDateEnum = QuarterDateEnum.getQuarterDateEnumByQuarter(quarter);
            String endTimeStr = year + "-" + quarterDateEnum.getEndDate();
    ...
    private BigDecimal getAttainProbability(String parentId, String broker, String analyst, Integer year, Integer quarter) {
        // analyst的名称需要去除末尾的数字，因为券商评分的同券商同行业不允许出现重名，如果重名在后面加数字
        try {
            QuarterDateEnum quarterDateEnum = QuarterDateEnum.getQuarterDateEnumByQuarter(quarter);
            String endTimeStr = year + "-" + quarterDateEnum.getEndDate();
